pub fn solve(part: u8, data: Result<String, std::io::Error>) {
    ...
    match part {
        ...
        2 => {
            let output = part_1::run(&mut program, 5);
            println!("{:?}", output);
        }
        _ => (),
    }
...
pub mod part_2 {
    ...
    mod tests {
        ...
        fn test_equal_position_mode() {
            ...
            let output = part_1::run(&mut program.clone(), 8);
            assert_eq!(output, Some(1));

            let output = part_1::run(&mut program.clone(), 9);
        ...
        fn test_less_than_position_mode() {
            ...
            let output = part_1::run(&mut program.clone(), 7);
            assert_eq!(output, Some(1));
        ...
        fn test_jump_position_mode() {
            ...
            let output = part_1::run(&mut program.clone(), 10);
            assert_eq!(output, Some(1));

            let output = part_1::run(&mut program.clone(), 0);
            assert_eq!(output, Some(0));
        }
